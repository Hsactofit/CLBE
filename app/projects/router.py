...
@router.get("/{project_public_id}", response_model=ProjectPublic)
async def get_project(project_state: ProjectState = Depends(get_project_state)):
    """Get one project for a client"""
    try:
        logger.info(f"Getting project: {project_state.project.public_id}")
